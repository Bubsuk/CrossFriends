...
public class TouchInput : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (GameManager.Instance.IsGameOver == false && (-25f < transform.position.x && transform.position.x < 25f))
        {
            ...
            {
                _curTouchPos = Input.GetTouch(0).position;
                Vector2 distance = _curTouchPos - _startTouchPos;

                if (!_stopTouch)
                {
                    if (distance.y > _swipeRange && _isJump == false)
                    {
                        // 위 스와이프
                        _stopTouch = true;
                    ...
                    else if (distance.y < -_swipeRange && _isJump == false)
                    {
                        // 아래 스와이프
                        _stopTouch = true;
                        bool result = false;
                    ...
                    else if (distance.x < -_swipeRange && _isJump == false)
                    {
                        // 왼쪽 스와이프
                        _stopTouch = true;
